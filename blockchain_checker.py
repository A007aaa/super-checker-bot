        ...
        if type == "SEED":
            ...
            try:
                ...
                if len(decoded) == 64:
                    ...
                    # Se for apenas a chave privada, o ideal seria derivar a pública, 
                    # mas por enquanto vamos registrar o log
                    logger.warning(f"Chave SOL de 32 bytes detectada, derivação pendente: {value}")
            except Exception as e:
                logger.error(f"Erro ao processar SOL_KEY: {e}")
            
        elif type == "ETH_KEY":
            # TODO: Adicionar derivação de endereço ETH a partir da chave privada hex
            pass
