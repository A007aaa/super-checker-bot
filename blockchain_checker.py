        ...
        for attempt in range(MAX_RETRIES + 1):
            try:
                ...
                async with req(url, timeout=timeout, headers=HEADERS, **kwargs) as res:
                    ...
                    if res.status == 429: await asyncio.sleep(5 * (attempt + 1))
            except: pass
...
async def check_trx_full(session, label, addr):
    ...
    if data and data.get('data'):
        ...
        if acc.get('balance', 0) > 0:
            results.append((f"TRX ({label})", addr, acc['balance'] / 10**6))
        # Saldo de Tokens TRC-20 (Melhorado)
            ...
            for contract, balance in token_dict.items():
                try:
                    ...
                    if val > 0:
                        ...
                        # Para USDT usamos 6 decimais, para outros mostramos o valor bruto se não soubermos
                        divisor = 10**6 if contract == usdt_contract else 1
                        results.append((f"{symbol} ({label})", addr, val / divisor))
                except: continue
    return results
